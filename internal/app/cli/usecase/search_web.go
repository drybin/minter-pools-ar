...
// nolint: unused
func generateTgMessageOther(
    coin string,
    response model.BuyRawResponse,
    commission float64,
    attempt int,
) string {
    newLine := "\n"
    ...
    return fmt.Sprintf(
        "*OTHER COIN* %s Баланс %.2f"+newLine+
            ...
            for _, price := range prices {
                ...
                if result < amountOut {
                    ...
                    if err != nil {
                        ...
                        result := res.AmountInFloat + *commission
                        fmt.Println("Second attempt")
                        fmt.Printf(
                            "AmountInFloat %f, comission: %f, result  %f, amountOut %f",
                            res.AmountInFloat,
                            *commission,
                            result,
                            amountOut,
                    ...
                    msg := generateTgMessageOther(coin, *res, *commission, attempt)
                    _, err = u.TgWebapi.Send(msg)
                    if err != nil {
                        return wrap.Errorf("failed to send TG message: %w", err)
                    }
